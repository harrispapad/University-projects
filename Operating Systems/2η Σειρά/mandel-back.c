...
void output_mandel_line(int fd, int color_val[])
{
    ...
    for (i = 0; i < x_chars; i++) {
        ...
        /* Output the point using printf */
        printf("%c", point);
    }

    /* Output a newline character using printf */
    printf("%c", newline);
}
...
void *thread_function(void *arg) {
   	int thread_num = *((int *)arg);
    	...
    	for (int line = thread_num; line < y_chars; line += NUM_THREADS ) {
		sem_wait(&proc[thread_num]);
		
        compute_and_output_mandel_line(thread_num, line);

        if (thread_num != NUM_THREADS - 1)
            sem_post(&proc[thread_num + 1]);
        else
		sem_post(&proc[0]);
