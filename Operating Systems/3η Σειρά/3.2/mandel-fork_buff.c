...
void compute_and_output_mandel_line(int line)
{
	 ...
	 */
        for (int i=line ; i<y_chars; i+=NUM_PROCS) {
                compute_mandel_line(i, buffer[i]);
        }
}
...
int main(int argc, char *argv[])
{
	 ...
	 */
        buffer = create_shared_memory_area(y_chars * sizeof(int)); //create the initial 1d array
	...
	for(int proc_num = 0; proc_num < NUM_PROCS; proc_num++) {
		...
		if(p < 0) {
			perror("Error during fork.\n");
				...
				compute_and_output_mandel_line(proc_num);
				exit(0);
			}
		}
